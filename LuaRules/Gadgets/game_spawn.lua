...
if (not gadgetHandler:IsSyncedCode()) then
	--Spring.Echo("Not synced code, exiting")
	return false
end
...
local function GetStartUnit(teamID)
	...
	--Spring.Echo("Start unit is "..startUnit)
	return startUnit
end

local function SpawnStartUnit(teamID)
	local luaAI = Spring.GetTeamLuaAI(teamID)
	if luaAI and string.find(string.lower(luaAI), "defenders") then return nil end

	--Spring.Echo("Attempting to spawn start unit for team "..teamID)
	...
	local m = 0 --teamOptions.startmetal  or modOptions.startmetal  or 1000
	local e = 0 --teamOptions.startenergy or modOptions.startenergy or 1000
...
function gadget:Initialize()
	--Spring.Echo("Spawn gadget initialized")
end

function gadget:GameStart()
	--Spring.Echo("Game start, attempting to spawn units")
	...
	for i = 1,#teams do
		...
		--Spring.Echo("Checking team "..teamID)
