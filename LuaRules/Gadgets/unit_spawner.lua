...
local eggs = false	--tobool(Spring.GetModOptions().eggs)
local pvp = false
...
local function KillAllComputerUnits()
  ...
  for teamID in pairs(computerTeams) do
    ...
    for i=1,#teamUnits do
      Spring.AddUnitDamage(teamUnits[i], 1000000)
    end
  end
end
...
local function SpawnTurret(burrowID, turret, number, force)
  ...
  for i=1, spawnNumber do  
	...
	if turretDef["customParams"]["type"] == "small" then
		spSpawnCEG("teleport_small", x, y, z)
	end
...
local function SpawnQueen()
  ...
  impSetupFrame = Spring.GetGameFrame() + 2
...
local function SetupImperator()
	if not queenID then return end
	local queenHardpoints = GG.imperators[queenID].hardpoints
	for index,turret in pairs(queenTurrets) do
		local hpID = queenHardpoints[index]
		GG.hardpoints[hpID][4] = UnitDefNames[turret].id
		GG.hardpoints[hpID][5] = 0
	end
	impSetupFrame = 999999999
end
...
function gadget:GameStart()
	...
	for _,perk in pairs(perks or {}) do
		GG.PickPerk(chickenTeamID, perk)
	end
end
...
function gadget:GameFrame(n)
  ...
  if queenID and (n > impSetupFrame) then
	SetupImperator()
  end
...
function gadget:GameOver()
	...
	--if ExceedsOne(modopts.metalmult) or ExceedsOne(modopts.metalmult)) or ExceedsOne(modopts.wreckagemult) or (not ExceedsOne(modopts.factorycostmult + 0.001)) then
...
--function RecvFromSynced(key, value)
--	if key == "queenID" then queenID = value end
--end

function gadget:RecvFromSynced(name, val)
	if name == "queenID" then queenID = val end
end
...
--drawing code copied from repair gadget by KDR_11k (David Becker)

local size=250

local function Circle()
	for i=0,39 do
		glVertex(math.cos(i*.05*3.1415)*size,0,math.sin(i*.05*3.1415)*size)
	end
end
...
function gadget:DrawWorldPreUnit()
	glTexture("bitmaps/flagship.tga")
	--glBlending(GL_SRC_ALPHA,GL_ONE_MINUS_SRC_ALPHA)
	glLineWidth(2)
	glColor(.6,.6,.2,1)
	if queenID then
		local x,y,z=spGetUnitPosition(queenID)
		local ateam = spGetLocalAllyTeamID()
		if x and y and z then
			local _,los = spGetPositionLosState(x,y,z,ateam)
			los = specView or (los and not spGetUnitIsCloaked(queenID))
			if spIsUnitVisible(queenID) and los then
				glPushMatrix()
				glTranslate(x,0,z)
				glCallList(commandShape)
				--Shape()
				glPopMatrix()
			end
		end
...
function gadget:Initialize()
  ...
  commandShape = glCreateList(Shape)
end



function gadget:Shutdown()
	glDeleteList(commandShape)
end
